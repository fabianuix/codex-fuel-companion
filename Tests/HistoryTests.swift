...
@main struct HistoryTests {
    ...
    static func main() throws {
        ...
        let allowanceHigh = limits(balance: "0", used: 70)
        let atQuarter = limits(balance: "0", used: 75)
        let belowTenth = limits(balance: "0", used: 91)
        precondition(AllowanceWarning.crossings(from: nil, to: atQuarter, thresholds: [25, 10]).isEmpty)
        let quarter = AllowanceWarning.crossings(from: allowanceHigh, to: atQuarter, thresholds: [25, 10])
        precondition(quarter.count == 1 && quarter[0].threshold == 25 && quarter[0].remaining == 25)
        precondition(AllowanceWarning.crossings(from: atQuarter, to: atQuarter, thresholds: [25, 10]).isEmpty)
        let tenth = AllowanceWarning.crossings(from: atQuarter, to: belowTenth, thresholds: [25, 10])
        precondition(tenth.count == 1 && tenth[0].threshold == 10)
        precondition(AllowanceWarning.crossings(from: allowanceHigh, to: belowTenth, thresholds: [25, 10]).first?.threshold == 10)
        precondition(AllowanceWarning.crossings(from: allowanceHigh, to: belowTenth, thresholds: [20]).first?.threshold == 20)
        precondition(AllowanceWarning.crossings(from: belowTenth, to: belowTenth, thresholds: [25, 10]).isEmpty)
        precondition(AllowanceWarning.crossings(from: allowanceHigh, to: limits(balance: "0", used: 80, account: "two"), thresholds: [25]).isEmpty)
        precondition(AllowanceWarning.crossings(from: allowanceHigh, to: limits(balance: "0", used: 80, reset: 5000), thresholds: [25]).isEmpty)
        precondition(AllowanceWarning.crossings(from: allowanceHigh, to: belowTenth, thresholds: [0, 100]).isEmpty)
        print("PASS: exact percentage crossings, 25/10/custom choices, large jumps, duplicate polls, account and reset-cycle isolation")
