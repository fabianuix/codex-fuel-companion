...
set -euo pipefail
cd "${0:A:h:h}"
mkdir -p .build/checks docs/plan-examples
...
xcrun swiftc -swift-version 5 -module-cache-path "$PWD/.build/ModuleCache" -F "$framework_parent" -framework Sparkle -Xlinker -rpath -Xlinker "$framework_parent" "${source_files[@]}" Tests/PlanExamples.swift -o .build/checks/PlanExamples
.build/checks/PlanExamples
